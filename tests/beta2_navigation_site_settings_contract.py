...
checks = {
    ...
    'canonical navigation renderer': all(token in nav_renderer for token in (
        'NavigationRepository::exists',
        'wp_nav_menu([',
        "'fallback_cb' => false",
        'data-vdm-navigation-toggle',
        'aria-expanded="false"',
        'data-vdm-navigation-panel',
    ...
    'navigation style variables': all(token in renderer for token in (
        '--vdm-navigation-gap:',
        '--vdm-navigation-font-size:',
        '--vdm-navigation-font-weight:',
        '--vdm-navigation-text:',
        '--vdm-navigation-hover:',
        '--vdm-navigation-submenu-background:',
    ...
    'Designer navigation defaults and inspector': all(token in designer for token in (
        ...
        "field('WordPress-menu'",
        "field('Retning'",
        "field('Hoverfarve'",
        "field('Mobilknap tekst'",
        'config.navigationMenus',
    )),
    'navigation responsive CSS': all(token in css for token in (
        '.vdm-navigation{',
        '.vdm-navigation-menu',
        '.vdm-navigation-toggle',
        '.vdm-navigation.is-open .vdm-navigation-panel',
    ...
    'navigation frontend runtime': all(token in frontend_js for token in (
        ...
        "aria-expanded",
        "event.key !== 'Escape'",
        "matchMedia('(min-width: 783px)')",
    )),
    'shared frontend asset registration': "wp_register_script(\n            'vdm-frontend-runtime'" in frontend and 'public static function enqueueAssets()' in frontend and "wp_enqueue_script('vdm-frontend-runtime')" in frontend,
    ...
    'site settings admin security': all(token in settings_controller for token in (
        ...
        "'manage_options'",
        "admin_post_vdm_save_site_settings",
    ...
    'site settings fields': all(token in settings_controller for token in (
        'Webstedstitel', 'Slogan', 'Virksomhed / forening', 'Kontakt-e-mail', 'Kontakttelefon', 'VDM-logo', 'Site-ikon / favicon', 'Hjemadresse', 'WordPress-adresse'
    )),
    ...
    'navigation admin page': "public const MENU_SLUG = 'vdm-navigation';" in nav_controller and 'NavigationRepository::choices()' in nav_controller and "nav-menus.php" in nav_controller,
